...
async function fetchPerformers() {
  ...
  const queryAllPerformers = `
    query FindPerformers {
      findPerformers(
        performer_filter: { name: { value: "\\\\s", modifier: MATCHES_REGEX } }
        filter: { per_page: -1 }
      ) {
        performers {
          name
  ...
  console.log(queryAllPerformers); // Print the attempted query
  ...
  try {
    ...
    console.log("Response from GraphQL server:");
    console.log(response.data); // Print the response from the server

    if (!response.data || !response.data.data || !response.data.data.findPerformers || !response.data.data.findPerformers.performers) {
      throw new Error("Response data is not in the expected format");
    }

    const allPerformers = response.data.data.findPerformers.performers;
...
async function processPerformer(page, performer) {
  ...
  const formattedName = `${firstName}-${lastName}`.toLowerCase();
