(async function() {
    ...
    const stashDBConfig = stashBoxes.find(box => box.endpoint === config.stashDBEndpoint)
    const tpdbConfig = stashBoxes.find(box => box.endpoint.startsWith("https://theporndb.net/graphql")); // support ?type=Movie
    const fansdbConfig = stashBoxes.find(box => box.endpoint === config.fansdbEndpoint || box.endpoint === "https://fansdb.xyz/graphql"); // support old FansDB domain
