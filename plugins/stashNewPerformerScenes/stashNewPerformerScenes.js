(async function () {
  ...
  const performGraphQLQuery = async (
    endpoint,
    query,
    variables = {},
    apiKey = null
  ...
  const getAllStashBoxes = async () => {
    ...
    const filteredStashBoxes = result.configuration.general.stashBoxes.filter(
      (box) => {
    ...
    console.log(
      'External StashBoxes retrieved (excluding ThePornDB):',
      config.stashBoxes
    );
  };
  ...
  const getStashDBApiKey = async () => {
    ...
    if (stashDBBox) {
      ...
      console.log(
        'StashDB API key retrieved successfully:',
        config.stashDBApiKey
      );
    } else {
      console.warn(
        'StashDB API key not found in external StashBoxes configuration'
      );
    }
  };
  ...
  const getPerformerStashIDs = async (performerIds) => {
    ...
    console.log(
      'Performer to StashID map:',
      JSON.stringify(performerStashMap, null, 2)
    );
  ...
  const getNewReleasesForStashIDs = async (performerStashMap) => {
    ...
    for (const stashBox of config.stashBoxes) {
      ...
      if (!api_key) {
        console.warn(
          `Skipping stashBox ${name} (${endpoint}) due to missing API key.`
        );
        continue;
      }
      ...
      console.log(
        `New Releases from ${name} (${endpoint}):`,
        JSON.stringify(stashScenesMap[endpoint], null, 2)
      );
    }
  ...
  const addNewReleaseIcon = (card, stashMap, stashScenesMap) => {
    ...
    if (card.querySelector('.new-release-bell-icon')) {
      console.log(
        'Bell icon already exists in this card. Skipping adding another.'
      );
      return;
    }
    ...
    iconContainer.href = `${firstEndpoint.replace(
      '/graphql',
      ''
    )}/performers/${firstStashId}`;
    ...
    if (thumbnailSection) {
      ...
      if (computedStyle.position === 'static') {
        ...
        console.log(
          '.thumbnail-section already has position:',
  ...
  const processPerformerCards = debounce(async () => {
    ...
    console.log(
      'StashID to Scenes map:',
      JSON.stringify(stashScenesMap, null, 2)
    );
    ...
    for (const performerId of performerIds) {
      ...
      Object.entries(performerStashScenesMap).forEach(([endpoint, scenes]) => {
        const filteredScenes = scenes.filter(
          (scene) => new Date(scene.release_date) > cutoffDate
        );
      ...
      console.log(
        `Performer ID: ${performerId}, New Releases: ${
          Object.keys(filteredStashScenesMap).length
        }`
      );
  ...
  const initializeNavbarObserver = () => {
    ...
    const observer = new MutationObserver(() => {
      ...
      if (navbarButtons) {
        console.log(
          '.navbar-buttons found, disconnecting observer and adding FA icon.'
        );
        observer.disconnect();
  ...
  const initializePageChangeListener = () => {
    ...
    PluginApi.Event.addEventListener('stash:location', () => {
      ...
      if (newPath.startsWith('/performers')) {
        console.log(
          'On performers page, initializing performer card processing.'
        );
  ...
  if (window.location.pathname.startsWith('/performers')) {
    console.log(
      'On performers page at initial load, processing performer cards.'
    );
    waitForElement('.performer-card', () => {
    ...
    console.log(
      'Not on a performers page at initial load. Skipping performer card processing.'
    );
  }
