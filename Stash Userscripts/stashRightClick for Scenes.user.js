...
// @downloadURL  https://github.com/Serechops/Serechops-Stash/raw/main/Stash%20Userscripts/stashRightClick%20for%20Scenes.user.js
// @updateURL    https://github.com/Serechops/Serechops-Stash/raw/main/Stash%20Userscripts/stashRightClick%20for%20Scenes.user.js
// @run-at       document-end
// ==/UserScript==
...
(async function() {
    ...
    async function updateSceneWithGalleries(sceneId, newGalleryIds) {
        ...
        sceneUpdate(input: { id: $scene_id, gallery_ids: $gallery }) {
            id
        }}`;
    ...
    async function fetchExistingTagIds(sceneId) {
        const query = `query ($id: ID!){ findScene(id: $id) {
            tags { id }}}`;
    ...
    async function fetchSceneStreams(sceneId) {
        const query = ` query ($id: ID!) { sceneStreams(id: $id) {
            url mime_type label }}`;
        const variables = { id: sceneId };
        return await fetchGQL(query, variables)
            .then(res => {
